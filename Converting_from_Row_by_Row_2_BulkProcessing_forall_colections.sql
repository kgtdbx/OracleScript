...
Procedure created.
Statement 4
The point here is that you need to do some "fancy" processing in PL/SQL that could not be (easily) done in SQL - thus, rather than rely on "pure" SQL to perform your bulk DML, you will need to use BULK COLLECT and FORALL.
...
Procedure created.
Statement 5
For each employee in department, give them the new salary. First, though, record the previous salary in the history table. Then call adjust_compensation to perform some incredibly complicated, procedural logic that may modify the new salary. Then do the update. Notice that if the insert fails, you never get to the update. And if either fails, you log the error and then continue.
The "Before" Version
...
Statement
7
To take things further, what if the current (non-bulk) code not only skips the update if the insert fails, but also rolls back to the insert if the update fails? How do we do the same thing with bulk processing?
