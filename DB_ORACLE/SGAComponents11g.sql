...
col setting format 999,999,999,999 head "Setting"

break on component nodup
select component, to_char(end_time, 'dd-MON-yyyy hh24:mi') lasttime, max(final_size) setting
from v$memory_resize_ops
group by component, end_time
union
select component, to_char(end_time, 'dd-MON-yyyy hh24:mi'), min(final_size)
from v$memory_resize_ops
group by component, end_time
order by 1, 3 desc
/
