...
Description
The LOG ERRORS feature allows you to suppress errors at the row level in DML statements. Instead of raising the error, the error information is written to a log table (generated by a call to DBMS_ERRLOG.CREATE_ERROR_LOG). But the design of the log table leaves a few things to be desired (doesn't include when the error occurred, "who" did it, and where in the code stack the statement was executed). This package helps "fill the gap" by creating the log table, altering it to add key columns and a trigger to set their values, and generate a package to make it easier to manage the table.
Category
PL/SQL General
Contributor
